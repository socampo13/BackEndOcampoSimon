...
app.use(
    session({
        store: connectMongo.create({
            mongoUrl:
            "mongodb+srv://simon:<password>@cluster0.3xmjz.mongodb.net/myFirstDatabase?retryWrites=true&w=majority",
...
app.get("/", (req, res) => {
    ...
    if(!req.session.name && logIn.isLogged){
        logIn.isLogged = false;
        logIn.isTimedOut = true;
        res.render(`${__dirname}/src/views/signin.hbs`, { login: logged });
        logIn.isTimedOut = false;
        logged.nombre = "";
    }
    if(!req.session.name){
        console.log("Not logged in", body.name);
        res.render(`${__dirname}/src/views/signin.hbs`, { login: logged });
        console.log("Couldn't log");
    }else{
        logIn.isLogged = true;
        logIn.name = req.session.name;
        console.log("LogIn name:", logIng.name, "","session name", req.session.name);
        res.render(`${__dirname}/src/views/main.hbs`, { name: logged });
    }
});
...
app.post("/logout", (req, res) => {
    req.session.destroy();
    logIn.isLogged = false;
    logIn.isDestroyed = true;
    res.redirect("/");
});

const authenticate = (req, res, next) => {
    if(req.session.name = logIn.name){
        return next();
    }else{
        return res.sendStatus(401);
    }
